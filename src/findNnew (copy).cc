/*
The current issue is in detection.  center.xyz shows that something is up with
either the generation or something; also, there needs to be a mechanism to
show if the total number of neighbors is greater than MAX_NN.  Only a few local
particles have trouble, however.
...
/*  void findCutoffSqr()
 *  
 *  Find the radius at which nearest neighbor bonds are found. ***
 */
void findCutoffSqr()
{   cutoffSqr = NEIGHBOR_SQUARE_CUTOFF; }
...
/*  double eucl_dist_squared(Particle&, Particle&)
 *  
 *  Return the squared distance by the Pythagorean theorem.
 */
inline double eucl_dist_squared(Particle& p1, Particle& p2)
{   double  x = p1.x - p2.x,
            y = p1.y - p2.y,
            z = p1.z - p2.z;
    return (x * x + y * y + z * z); }
...
{   if(accumulate(p1.countN.begin(), p1.countN.begin() + dissolnStates, 0) >= maxNN)
    {   char err[64];
        sprintf(err, "Too many neighbors detected for particle %d.", p1.id);
        warning(err); }
    if(accumulate(p2.countN.begin(), p2.countN.begin() + dissolnStates, 0) >= maxNN)
    {   char err[64];
        sprintf(err, "Too many neighbors detected for particles %d.", p2.id);
        warning(err); }
...
{   for(uint i = 0; i < cell1.size(); i++)
    {   /*if (cell1[i].id == 257645) cell1[i].output();
        if (cell1[i].id == 303347) cell1[i].output();
        if (cell1[i].id == 260203) cell1[i].output();
        if (cell1[i].id == 260202) cell1[i].output();
        if (cell1[i].id == 260199) cell1[i].output();
        if (cell1[i].id == 260049) cell1[i].output();
        if (cell1[i].id == 257650) cell1[i].output();
        if (cell1[i].id == 257648) cell1[i].output();
        if (cell1[i].id == 257647) cell1[i].output();
        if (cell1[i].id == 257646) cell1[i].output();
        if (cell1[i].id == 257644) cell1[i].output();
        if (cell1[i].id == 257496) cell1[i].output();
        if (cell1[i].id == 257493) cell1[i].output();*/ //***
        for(uint j = i + 1; j < cell1.size(); j++)
        {   if(eucl_dist_squared(cell1[i], cell1[j]) <= cutoffSqr)
...
{   for(uint i = 0; i < cell1.size(); i++)
    {   for(uint j = 0; j < cell2.size(); j++)
        {   if(eucl_dist_squared(cell1[i], cell2[j]) <= cutoffSqr)
    ...
    {   for(uint j = 0; j < numCellsY; j++)
        {   for(uint k = 0; k < numCellsZ; k++)
            {   //  Output progress message.
                if (verbose)
                {   currProg= i * numCellsX * numCellsY + j * numCellsY + k;
                    maxProg = numCellsX * numCellsX * numCellsY +
                              numCellsY * numCellsY + numCellsZ;
                    if (currProg % (int) (0.25f * maxProg) == 0 &&
                        currProg > 0 && currProg != maxProg)
                    {   cout << rank << ":  "
                             << (int) (100 * (double) currProg / (double) maxProg)
                             << "% complete with local neighbors." << endl; } }
 ...
 */
inline void checkNeighbors(Cell& cell, coord_t* data)
{   Particle other(data[0], data[1], data[2], data[3], data[4]);
    
    for(uint i = 0; i < cell.size(); i++)
    {   if(eucl_dist_squared(cell[i], other) <= cutoffSqr)
        {   makeNeighborsAffectOnlyLeft(cell[i], other); } } }
 ...
 */
void calculateNeighbors()
{   findCutoffSqr();        if (verbose && !rank) cout << "Cutoff distance at " << sqrt(cutoffSqr) << endl;
    splitIntoCells();       if (verbose) cout << rank << ":  Cell division complete." << endl;
    findLocalNeighbors();   if (verbose) cout << rank << ":  Local neighbors found." << endl;
    /*findRemoteNeighbors();  if (verbose) cout << rank << ":  Remote neighbors found." << endl;
    fillMap();              if (verbose) cout << rank << ":  Particles inserted into map." << endl;
    */
}
